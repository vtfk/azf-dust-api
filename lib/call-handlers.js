...
const { ad, aad, feide, vis, sds, visma, sync, equitrack } = require('../systems')
...
module.exports = async (caller, body, name) => {
  try {
    ...
    // handle request for EquiTrack
    if (name.toLowerCase() === 'equitrack') {
      return await equitrack.handle(caller, body)
    }
