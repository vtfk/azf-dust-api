module.exports.generate = (system, user) => {
  ...
  if (propPriority.length === 0 && system !== 'vigobas') throw new Error(`System '${system}' not found`)
  ...
  if (Object.getOwnPropertyNames(query).filter(prop => !['domain', 'type'].includes(prop)).length === 0 && system !== 'vigobas') throw new Error('Ikke nok informasjon tilstede for å hente data')
