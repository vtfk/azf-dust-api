 ...
 */
module.exports.isApprentice = user => hasData(user) && hasData(user.type) && user.type === 'Lærling'
 ...
 */
module.exports.isOT = user => hasData(user) && hasData(user.type) && user.type === 'Elev-OT'

/**
 * Checks if user is a student (@skole.vtfk.no)
 * @returns {boolean} Is a student or not
 */
module.exports.isStudent = user => hasData(user) && hasData(user.type) && user.type === 'Elev'
