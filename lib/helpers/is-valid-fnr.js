...
module.exports = digits => {
  if (digits.length !== 11) return { valid: false, error: 'Fødselsnummeret har ugyldig lengde' }

  const isDnr = parseInt(digits.substring(0, 1)) > 3
  if (isDnr) digits = (digits.substring(0, 1) - 4) + digits.substring(1)

  const isVnr = isDnr && parseInt(digits.substring(6, 2)) === 99
  if (isVnr) return { valid: true, type: 'VIGO-nummer' }

  const type = isDnr ? 'D-nummer' : 'Fødselsnummer'

  const validDate = validateDate(digits)
