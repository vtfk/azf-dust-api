...
module.exports.getVigoUser = async (query, type) => {
  const collectionName = type === 'Elev-OT' ? MONGO.COLLECTION_VIGO_OT : type === 'Lærling' ? MONGO.COLLECTION_VIGO_LAERLING : MONGO.COLLECTION_USERS
  const dbName = type === 'Elev-OT' ? MONGO.NAME_VIGO_OT : type === 'Lærling' ? MONGO.NAME_VIGO_LAERLING : MONGO.NAME
  const db = await mongo(collectionName, dbName)
  logger('info', ['handle-mongo', 'get-vigo-user', type, 'start'])
  const user = await db.findOne(query)
  logger('info', ['handle-mongo', 'get-vigo-user', type, 'finish'])
  return user
}
