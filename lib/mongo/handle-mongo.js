...
module.exports.updateRequest = async request => {
  const { instanceId, name, data, user, status, error, innerError, tests, timestamp, started, finished, runtime, runtimeRequest } = request
  const setQuery = { $set: {} }
  ...
  if (runtimeRequest !== undefined) setQuery.$set.runtime = runtimeRequest
...
module.exports.getVigoUser = async (query, title) => {
  const collectionName = title === 'Elev-OT' ? MONGO.COLLECTION_VIGO_OT : title === 'Lærling' ? MONGO.COLLECTION_VIGO_LAERLING : MONGO.COLLECTION_USERS
  const dbName = title === 'Elev-OT' ? MONGO.NAME_VIGO_OT : title === 'Lærling' ? MONGO.NAME_VIGO_LAERLING : MONGO.NAME
  const db = await mongo(collectionName, dbName)
  logger('info', ['handle-mongo', 'get-vigo-user', title, 'start'])
  const user = await db.findOne(query)
  logger('info', ['handle-mongo', 'get-vigo-user', title, 'finish'])
  return user
}
