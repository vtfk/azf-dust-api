...
module.exports.updateRequest = async request => {
  const db = await mongo()
  const { instanceId, name, data, user, status, error, innerError, test, finishTimestamp } = request
  let setQuery = { $set: {} }
  let pushQuery = { $push: {} }
  ...
  if (finishTimestamp !== undefined) setQuery.$set.finished = finishTimestamp
  if (test !== undefined) {
