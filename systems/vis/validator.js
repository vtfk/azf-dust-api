...
module.exports = (systemData, user, allData = false) => ([
  ...
  test('vis-14', 'Har feidenavn', 'Sjekker at feidenavn er skrevet tilbake i ViS', () => {
    ...
    if (user.expectedType === 'student') {
      if (systemData.person.elev.feidenavn && systemData.person.elev.feidenavn.identifikatorverdi) return systemData.person.elev.feidenavn.identifikatorverdi === allData.feide.eduPersonPrincipalName ? success({ message: `${systemNames.feide}-navn er skrevet tilbake til ${systemNames.vis}`, raw: { vis: systemData.person.elev.feidenavn.identifikatorverdi, feide: allData.feide.eduPersonPrincipalName } }) : error({ message: `${systemNames.feide}-id skrevet tilbake er ikke riktig 😱`, raw: { vis: systemData.person.elev.feidenavn, feide: allData.feide.eduPersonPrincipalName }, solution: 'Meld sak til arbeidsgruppe identitet' })
